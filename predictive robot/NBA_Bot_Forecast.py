...
class NBAForecast(object):
    '''
    預測NBA
    '''
    
    def __init__(self):
        self.account = ["Andy0512","Bob7777","si845221","Dan0819","Eric4123","Geraldine","Humphrey454","Madeleine","un77ef8",
                        "Leonardqq","oo7844512","Reginald7a","Olympia666","Theodoreyyy","qaswwsaq"]
    ...
    def found(self):
        ...
        if self.update == False:
            print("*********************Found NBA Data start*********************")   
            try:
                url = f'https://ecocoapidev1.southeastasia.cloudapp.azure.com/MatchEntryInfo/DateBetween/NBA/{self.date}~{self.date}'
                ...
                df_group = df.groupby(by=["EventCode","GroupOptionCode","MatchTime"])
                self.update = True
                print("*********************Found NBA Data successful!!*********************")  
                return df_group
    ...
    def bot_predict(self,df_group):
        ...
        if self.update == True and self.successful == False:
            ...
            try:
                ...
                print("*********************Bot Predict NBA successful!!*********************")  
            except Exception as e: 
                print(repr(e))
                print("機器人異常")
    ...
    def logfile(self,value,bot):
        ...
        for line in read:
            ...
            if bot in text and evencode in text and time < (
                    datetime.datetime.now() - datetime.timedelta(minutes=1)).strftime("%H:%M:%S,000"):
