...
class NBAPredict(object):
    ...
    def __init__(self):
        ...
        self.path = r'C:/Users/user/NBA預測/20221217/database/' + str((self.matchdate).strftime("%Y%m%d"))
        self.time = (datetime.now()).strftime("%Y%m%d%H%M")
        self.path_b = r'C:/Users/user/NBA預測/20221217/database/' + str((self.matchdate - timedelta(days=1)).strftime("%Y%m%d"))
        self.name = {
    ...
    def Lineups(self):
        ...
        for g in range(len(game)):
            time_g = ((self.matchdate-timedelta(days=1)).strftime("%Y-%m-%d")) + " " + game[g].find("div",{"class":"lineup__meta flex-row"}).find('div').text.replace(" ET","")
        ...
        if __name__ == '__main__':
            ...
            for i in range(0,len(player_list),5):
                ...
                if (i+5) >= len(player_list):
                    ...
                    player_event.to_excel(f"{self.path}/{self.time}/playername.xlsx")
                    return player_event
    ...
    def update_player(self,player_event):  
        ...
        if __name__ == '__main__':
            ...
            for i in range(0,len(player),5):
                tasks = [asyncio.ensure_future(callurl_and_getdata(event)) for event in range(i,i+5)]
                loop.run_until_complete(asyncio.wait(tasks))
                if (len(data_all) != 0) and ((len(data_all) % 50 == 0) or ((i+5) >= len(player))):
                    for html in range(len(data_all)):
                        data_html = []
    ...
    def NBA_predict(self):
        ...
        odds_all = odd_b.merge(odds_all,on=['Matchtime','Away','Home','Eventcode_x'])
        ...
        for p in pred:
            pre.append(p[1])
        match = (datetime.now()+timedelta(days=0)).strftime("%Y-%m-%d")
